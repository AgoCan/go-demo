...
func (x *DasRequest) String() string {
...
// DasServiceClient is the client API for DasService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://godoc.org/google.golang.org/grpc#ClientConn.NewStream.
type DasServiceClient interface {
	// Sends a greeting
	Das(ctx context.Context, in *DasRequest, opts ...grpc.CallOption) (*DasReply, error)
}

type dasServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewDasServiceClient(cc grpc.ClientConnInterface) DasServiceClient {
	return &dasServiceClient{cc}
}

func (c *dasServiceClient) Das(ctx context.Context, in *DasRequest, opts ...grpc.CallOption) (*DasReply, error) {
	out := new(DasReply)
	err := c.cc.Invoke(ctx, "/das.DasService/Das", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// DasServiceServer is the server API for DasService service.
type DasServiceServer interface {
	// Sends a greeting
	Das(context.Context, *DasRequest) (*DasReply, error)
}

// UnimplementedDasServiceServer can be embedded to have forward compatible implementations.
type UnimplementedDasServiceServer struct {
}

func (*UnimplementedDasServiceServer) Das(context.Context, *DasRequest) (*DasReply, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Das not implemented")
}

func RegisterDasServiceServer(s *grpc.Server, srv DasServiceServer) {
	s.RegisterService(&_DasService_serviceDesc, srv)
}

func _DasService_Das_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(DasRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(DasServiceServer).Das(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/das.DasService/Das",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(DasServiceServer).Das(ctx, req.(*DasRequest))
	}
	return interceptor(ctx, in, info, handler)
}

var _DasService_serviceDesc = grpc.ServiceDesc{
	ServiceName: "das.DasService",
	HandlerType: (*DasServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Das",
			Handler:    _DasService_Das_Handler,
		},
	},
